...
import { LlmFidClient, type FetchOptions } from "@llm-txt/sdk";
import {
  TerminalLayout,
  TerminalForm,
  TerminalRow,
...
function App() {
  ...
  const sdkClient = useMemo(() => {
    return new LlmFidClient({ baseUrl: SERVER_URL, signer: terminal.wallet.signer });
  ...
  useEffect(() => {
    ...
    const isFid = /^\d+$/.test(terminal.debouncedInput.trim());
    const options: FetchOptions = {
      ...params,
      ...(isFid
        ? { fid: Number(terminal.debouncedInput) }
        : { username: terminal.debouncedInput.trim().toLowerCase() }),
    };
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (isFid) {
        queryParams.set("fid", terminal.debouncedInput.trim());
      } else {
        queryParams.set("username", terminal.debouncedInput.trim().toLowerCase());
      }
      ...
      setGeneratedUrl(`${SERVER_URL}/fid?${queryParams.toString()}`);
    } catch {
      setGeneratedUrl(null);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const isFid = /^\d+$/.test(terminal.input.trim());
      const options: FetchOptions = {
        ...params,
        ...(isFid ? { fid: Number(terminal.input) } : { username: terminal.input.trim().toLowerCase() }),
      };
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    const isFid = /^\d+$/.test(terminal.input.trim());
    a.download = isFid ? `fid-${terminal.input.trim()}.txt` : `${terminal.input.trim()}.txt`;
  ...
  const stats = [];
  if (terminal.debouncedInput && serverEstimate?.castCount != null) {
    stats.push({ label: "CASTS", value: serverEstimate.castCount });
  }
    ...
    <TerminalLayout
      ...
      queryForm={
        <TerminalForm onSubmit={handleSubmit}>
          ...
          <PricingTable pricing={pricing?.farcaster} loading={pricingLoading} />
        </TerminalForm>
      }
    />
  );
